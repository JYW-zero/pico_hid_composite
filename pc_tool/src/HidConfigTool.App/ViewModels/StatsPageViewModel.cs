...
public partial class StatsPageViewModel : ObservableObject
{
        ...
        for (int row = 0; row < 8; row++)
        {
            for (int col = 0; col < 8; col++)
            {
                ...
                HeatmapKeys.Add(new HeatmapKeyViewModel
                {
                    ...
                    KeyName = name,
                    Count = 0,
                    BackgroundBrush = new SolidColorBrush(Color.FromRgb(26, 40, 59)) // 深色背景
                });
            }
        }
    }
    ...
    private async Task RefreshStatsAsync()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < 64 && i < HeatmapKeys.Count; i++)
            {
                ...
                key.BackgroundBrush = new SolidColorBrush(Color.FromRgb(r, g, b));
            }
    ...
    {
        var result = System.Windows.MessageBox.Show("确定要重置所有统计数据吗？", "确认重置",
