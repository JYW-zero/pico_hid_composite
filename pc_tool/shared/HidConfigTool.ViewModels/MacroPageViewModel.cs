...
public partial class MacroPageViewModel : ObservableObject
{
    ...
    private readonly ITimerService _recordTimer;
    ...
    public MacroPageViewModel(IDeviceService deviceService, IMacroRecorder macroRecorder,
        ITimerService timerService, IDialogService dialogService, IUiThreadService uiThreadService)
    {
        ...
        _recordTimer = timerService;
        _recordTimer.Interval = TimeSpan.FromMilliseconds(100);
    ...
    private void ClearMacro()
    {
        ...
        {
            if (_dialogService.ShowConfirm("确定要清空当前宏的所有动作吗？", "确认清空"))
            {
    ...
    private void AddKeyAction()
    {
        ...
        {
            _dialogService.ShowInfo($"最多只能添加 {MaxActions} 个动作", "提示");
            return;
        }
    ...
    private void ClearActions()
    {
        ...
        if (_dialogService.ShowConfirm("确定要清空所有动作吗？", "确认"))
        {
            Actions.Clear();
            SelectedAction = null;
            UpdateTotalDuration();
    ...
    {
        // 录制过程中实时更新 UI
        _uiThreadService.Invoke(() =>
        {
            if (Actions.Count >= MaxActions)
            {
                // 达到上限，自动停止录制
                StopRecording();
                _dialogService.ShowInfo($"已达到最大动作数 {MaxActions}，录制已自动停止", "提示");
                return;
            }
    ...
    private async Task PlayMacro()
    {
        ...
        {
            _dialogService.ShowInfo("设备未连接或未选择宏", "提示");
            return;
        }
        ...
        if (macroId >= 8)
        {
            _dialogService.ShowError("无效的宏ID", "错误");
            return;
        }
        ...
        {
            _dialogService.ShowError("播放宏失败", "错误");
        }
    }
    ...
    private async Task LoadMacrosFromDeviceAsync()
    {
        ...
        {
            _dialogService.ShowError($"加载宏失败: {ex.Message}", "错误");
        }
        finally
        {
            IsLoading = false;
    ...
    private async Task SaveCurrentMacroAsync()
    {
        if (CurrentMacro == null || !_deviceService.IsConnected)
        {
            _dialogService.ShowInfo("设备未连接", "提示");
            return;
        }
        ...
        try
        {
            ...
            if (ok)
            {
                _dialogService.ShowInfo("保存成功！", "提示");
            }
            else
            {
                _dialogService.ShowError("保存失败", "错误");
            }
        }
        catch (Exception ex)
        {
